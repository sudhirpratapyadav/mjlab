...
class ToolPullClassicalPolicy(ClassicalPolicyBase):
  ...
  def _observe(self, i: int, obs_i: np.ndarray) -> None:
    ...
    raw_gtt = obs_i[52:55]
    row1 = obs_i[55:58]
    # The cylindrical shaft can roll: R[1,1] is not cos(yaw) after a roll.
    # Recover its +x axis from the observed last two rotation-matrix rows.
    row0 = np.cross(row1, obs_i[58:61])
