...
class PegInsertionClassicalPolicy(GraspTransportPolicy):
  ...
  def _target_error(self, i: int, obs_i: np.ndarray):
    ...
    position, rotation = self._fk(q)
    if phase in (1, 2):
      # Brake descent using arm height, independently of a moved or tilted peg.
      rate = 0.006 if position[2] < 0.15 else 0.02
      err[2] = max(self.lift_grasp_site_z - position[2], -rate)
    err[2] = max(err[2], 0.012 - lowest_hand_z(position, rotation, max(q[7], q[8])))
    return err, rot, grip
