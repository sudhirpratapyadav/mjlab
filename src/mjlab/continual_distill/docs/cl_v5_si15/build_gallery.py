...
<div class="stats"><div class="stat a"><strong>0.795 &plusmn; 0.041</strong><span>Random ordering, 3 seeds</span></div><div class="stat"><strong>0.738 &plusmn; 0.049</strong><span>Fragile-first ordering, 3 seeds</span></div><div class="stat b"><strong>15</strong><span>Tasks, one shared network</span></div></div>
</header>
<div class="curve"><b>Scalability curve</b> (final average success across the full sequence): N=4 &rarr; <b>0.960</b> &middot; N=6 &rarr; <b>0.792</b> &middot; N=15 &rarr; <b>0.74&ndash;0.80</b> (this run). Roughly flat from N=6 to N=15, better than a naive extrapolation of the earlier N=4&rarr;N=6 drop would suggest.</div>
<p class="explanation">Videos below show the best run (random ordering, seed 2, final average 0.837) after training on all 15 tasks in sequence &mdash; a success episode where available, otherwise the best-return attempt. Per-task rate is that run's final environment evaluation after the whole sequence finished, not a standalone-teacher number. PushCuboid, ThrowToBin and OpenDrawer were consistently the hardest tasks to retain across every ordering and seed tried, independent of where they sat in the sequence &mdash; a fragility pattern, not just an ordering effect. A fragile-last ordering (testing whether mid-sequence placement is specifically bad at this scale) is running as a follow-up.</p>
<div class="grid">
{"".join(cards)}
</div>
<footer>Detailed per-run results, all 6 seeds/orderings, and training curves are in the linked W&amp;B project. This page reflects the current best single run; it will be refreshed if the fragile-last follow-up does better.</footer>
</main></body></html>'''
