...
def main():
  ...
  for record in evaluation['records']:
    ...
    if lane%4==0:
      restore(closest['step'],lane);data.qpos[fingers]=.035
      ...
      row['endpoint_ik']=min(cases,key=lambda r:(not r['passes'],r['position_error_m']+.001*r['orientation_error_deg']))
    rows.append(row)
  summary={k:float(np.median([r['closest'][k] for r in rows])) for k in ['distance_m','closing_alignment','approach_alignment','full_rotation_error_deg','tilt_deg']}
