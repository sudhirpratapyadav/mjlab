...
def main():
  ...
  for lane in range(len(source['lanes'])):
    ...
    opening=float(np.clip(d.qpos[fingers].mean(),0,.04));height=float(d.xpos[board,2]);cases=[]
    for mode in ['fixed_hand_close','lower_pad_preserving_close']:
      closed=start.copy()
      if mode=='lower_pad_preserving_close':closed-=(opening-.003)*rotation[:,1]
      ...
      mujoco.mj_forward(m,d);d.qacc_warmstart[:]=0;samples=[];minimum=0.;finite=True
      for tick in range(600):
        ...
        d.ctrl[7]=(1-min(time,1))*opening+min(time,1)*.003
  ...
  a.output.write_text(json.dumps(dict(task='Mjlab-Pivot-Lift-Franka',checkpoint_sha256=str(source['checkpoint_sha256']),source_controls=str(a.controls.resolve()),sample_archive=str(archive),sample_columns=['time_s','lift_m','opposed','tilt_deg','finger1','finger2'],method='All32 original bounded-path cases, native CPU, original states/friction. After full5s diagnostic pivot path:1s close to3mm,1s100mm lift,1s hold. Compare fixed hand and lower-pad-preserving hand shift. Bounded IK physical controls, no policy data/reset demonstrations/certification.',summary=summary,rows=rows),indent=2)+'\n')
