...
def main():
  ...
  try:
    for label,ev in [('before',baseline),('after',candidate)]:
      with np.load(Path(ev['trace_dir'])/'trace.npz') as z:t={k:z[k] for k in ['qpos','qvel','mocap_pos','mocap_quat','origins']}
      for lane in [0,1,2]:
        ...
        if label=='before':initial[lane]={k:v[0].copy() for k,v in states.items()}
        else:
  ...
  (a.output/'videos.json').write_text(json.dumps(dict(task=baseline['task'],seed=baseline['seed'],paired_initial_states_verified=True,method='Fixed preregistered envs0/1/2, identical initial states; each recorded20ms state rendered once at50fps. No resimulation/interpolation/time scaling or video smoothing.',clips=rows),indent=2)+'\n')
