...
expect(await page.locator('.task:visible').count()===16,'Expected 16 certified cards');
await page.locator('[data-filter="progress"]').click();
expect(await page.locator('.task:visible').count()===8,'Expected 8 unfinished cards');
...
fs.writeFileSync(output+'.json',JSON.stringify({url,verified_utc:new Date().toISOString(),cards:24,certified:16,in_progress:8,filters:true,search:true,mobile_overflow:false,browser_errors:errors,videos},null,2)+'\n');
await browser.close();
