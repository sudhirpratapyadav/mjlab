...
expect(await page.locator('.task:visible').count()===18,'Expected 18 certified cards');
await page.locator('[data-filter="progress"]').click();
expect(await page.locator('.task:visible').count()===6,'Expected 6 unfinished cards');
...
fs.writeFileSync(output+'.json',JSON.stringify({url,verified_utc:new Date().toISOString(),cards:24,certified:18,in_progress:6,filters:true,search:true,mobile_overflow:false,browser_errors:errors,videos},null,2)+'\n');
await browser.close();
