...
expect(await page.locator('.task:visible').count()===14,'Expected 14 certified cards');
await page.locator('[data-filter="progress"]').click();
expect(await page.locator('.task:visible').count()===10,'Expected 10 unfinished cards');
...
fs.writeFileSync(output+'.json',JSON.stringify({url,verified_utc:new Date().toISOString(),cards:24,certified:14,in_progress:10,filters:true,search:true,mobile_overflow:false,browser_errors:errors,videos},null,2)+'\n');
await browser.close();
