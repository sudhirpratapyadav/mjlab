...
expect(await page.locator('.task:visible').count()===15,'Expected 15 certified cards');
await page.locator('[data-filter="progress"]').click();
expect(await page.locator('.task:visible').count()===9,'Expected 9 unfinished cards');
...
fs.writeFileSync(output+'.json',JSON.stringify({url,verified_utc:new Date().toISOString(),cards:24,certified:15,in_progress:9,filters:true,search:true,mobile_overflow:false,browser_errors:errors,videos},null,2)+'\n');
await browser.close();
