...
class PegInsertionClassicalPolicy(GraspTransportPolicy):
  ...
  def _gto(self, i: int, obs_i: np.ndarray) -> np.ndarray:
    """Average a held tip in hand coordinates, and track it live after release."""
    phase = self._phase[i]
    if phase < P_LIFT:
      self._tip_count[i] = 0
      return super()._gto(i, obs_i)
    raw = obs_i[self.gto_slice]
    _, rotation = self._fk(self.default_qpos + obs_i[:9])
    held = phase < P_RELEASE
    sample = rotation.T @ raw if held else raw
    stamp = (int(phase), int(self._phase_steps[i]))
    if stamp != self._tip_stamp[i]:
      if held != self._tip_held[i]:
        self._tip_count[i] = 0
      self._tip_count[i] = min(self._tip_count[i] + 1, 20)
      alpha = 1.0 / self._tip_count[i] if held else 0.35
      if self._tip_count[i] == 1:
        self._tip_mean[i] = sample
      else:
        self._tip_mean[i] += alpha * (sample - self._tip_mean[i])
      self._tip_held[i] = held
      self._tip_stamp[i] = stamp
    return rotation @ self._tip_mean[i] if held else self._tip_mean[i].copy()
  ...
  def reset(self, env_ids=None) -> None:
    super().reset(env_ids)
    if env_ids is None:
      self._tip_mean = np.zeros((self.num_envs, 3))
      self._tip_count = np.zeros(self.num_envs)
      self._tip_held = np.zeros(self.num_envs, dtype=bool)
      self._tip_stamp = [None] * self.num_envs
      ...
      self._tip_count[env_ids] = 0
      for i in env_ids:
        self._tip_stamp[i] = None
