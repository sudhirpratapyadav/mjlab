...
class ReorientObjectClassicalPolicy(ClassicalPolicyBase):
  ...
  def _target_error(self, i: int, obs_i: np.ndarray):
    ...
    if ph >= P_ROTATE and self._phase_steps[i] > 12:
      # Separation is decisive; aperture alone can shrink as a retained bottle
      # rolls from an end-face pinch toward a narrower barrel contact.
      lost = np.linalg.norm(gto) > 0.08
      if lost and self._attempts[i] < MAX_ATTEMPTS:
        self._rewind(i, "lost during rotation")
    ...
    if ph == P_LIFT:
      if self._phase_steps[i] >= LIFT_STEPS:
        ...
        p, _R = self._fk(q_abs)
        self._hold[i] = p.copy()  # ABSOLUTE site hold while the hand reorients (a zero
        self._goto(i, P_ROTATE)   # error follows the sag: 9/32 floor hits in ROTATE)
        return np.zeros(3), rot, GRIPPER_CLOSED
      return np.array([0.0, 0.0, LIFT_HEIGHT]), rot, GRIPPER_CLOSED
    ...
    if ph == P_SETTLE:
      # The lift and wrist turn can move the bottle outside the in-place bound.
      # Bring it back using the observed spawn-anchor vector while keeping the
      # upright grip. Leave 6 cm of margin for hand/object offset and sensor noise.
      to_anchor = obs_i[43:45]
      if np.linalg.norm(to_anchor) > 0.12:
        self._hold[i, :2] += np.clip(0.03 * to_anchor, -0.003, 0.003)
    p, _R = self._fk(q_abs)
