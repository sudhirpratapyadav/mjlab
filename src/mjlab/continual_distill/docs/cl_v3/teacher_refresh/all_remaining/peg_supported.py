...
class PegInsertionClassicalPolicy(StagedGrasp):
  ...
  def _target_error(self,i,obs):
    ...
    if ph<4 or self.rot_n[i]==0:
      yaw=quat_yaw_wxyz(quat)
      current=np.arctan2(hand[1,1],hand[0,1])
      yaw+=np.round((current-yaw)/(np.pi/2))*(np.pi/2)
      axis=closing_frame(yaw)
      self.orientation_weight=.5;self.posture_weight=.0005
    else:
    ...
    if ph==0 and np.trace(hand.T@axis)<2.98:
      self._phase[i]=0
