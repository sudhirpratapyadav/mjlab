...
class RotateValveClassicalPolicy(ClassicalPolicyBase):
  ...
  def reset(self, env_ids=None) -> None:
    super().reset(env_ids)
    if env_ids is None:
      self._center = np.zeros((self.num_envs,3))
      self._center_n = np.zeros(self.num_envs,int)
  ...
  def _target_error(self, i: int, obs_i: np.ndarray):
    ...
    ee = self._fk(self.default_qpos + obs_i[:9])[0]
    center = ee + gto_raw + rz @ hinge_m
    if self._center_n[i] == 0:self._center[i] = center
    else:self._center[i] += (center-self._center[i])/min(100,self._center_n[i]+1)
    self._center_n[i] += 1
    gto = self._center[i] - ee - rz @ hinge_m
