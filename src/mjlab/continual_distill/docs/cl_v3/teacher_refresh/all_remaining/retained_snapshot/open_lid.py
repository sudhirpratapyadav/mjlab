...
class OpenLidClassicalPolicy(ClassicalPolicyBase):
  ...
  def reset(self, env_ids=None) -> None:
    ...
    if env_ids is None:
      ...
      self._world_est = np.zeros((self.num_envs, 3))
      self._world_n = np.zeros(self.num_envs, dtype=int)
    else:
  ...
  def _target_error(self, i: int, obs_i: np.ndarray):
    ...
    if self._phase[i] <= 2:
      ...
      sample = ee + gto_raw
      if self._world_n[i] == 0:
        self._world_est[i] = sample
      else:
        self._world_est[i] += .12 * (sample - self._world_est[i])
      self._world_n[i] += 1
      gto = self._world_est[i] - ee
    else:
      self._world_n[i] = 0
    rz = rot_z_mat(psi)
