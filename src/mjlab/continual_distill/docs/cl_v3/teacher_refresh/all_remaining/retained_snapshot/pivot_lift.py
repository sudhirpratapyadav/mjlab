...
class PivotLiftClassicalPolicy(ClassicalPolicyBase):
  ...
  def _act_single(self, i: int, obs_i: np.ndarray) -> np.ndarray:
    ...
    if ph != P_LIFT:
      # Joint-7 servo on the CLOSING axis, generalised from the hand-down version: rotate
      # about the approach axis until the site y-axis lines up with the target closing
      # axis (mod pi -- the pads are symmetric).  A yaw-CONSTRAINED DLS target was measured
      # to wreck the position tracking at this stretch.
      _p, R = self._fk(q_abs)
      a_cur, c_cur = R[:, 2], R[:, 1]
      c_des = self._c_target[i] - float(np.dot(self._c_target[i], a_cur)) * a_cur
      n = float(np.linalg.norm(c_des))
      if n > 1e-6:
        c_des = c_des / n
        ang = float(np.arctan2(float(np.dot(np.cross(c_cur, c_des), a_cur)),
                               float(np.dot(c_cur, c_des))))
        ang = float(np.angle(np.exp(2j * ang)) / 2.0)
        dq7 = float(np.clip(ang, -YAW_RATE, YAW_RATE))
