#!/usr/bin/env python3
"""Evaluate a classical teacher on independent, uninterrupted first episodes.

Use --legacy-window only to reproduce historical fixed-window retry rates.
...
from mjlab.continual_distill.classical import CLASSICAL_POLICIES as POLICIES
...
def main() -> None:
  ...
  parser.add_argument("--legacy-window", action="store_true",
                      help="Historical retry-based diagnostic, not the published success rate.")
  args = parser.parse_args()

  configure_torch_backends()
  if not args.legacy_window:
    from mjlab.continual_distill.classical.render_rollout import run_stats_phase
    stats = run_stats_phase(args.task, args.num_envs * args.num_episodes,
                            args.num_envs, args.device, args.seed)
    print("Protocol: single_episode_no_reset_retries")
    print(f"OVERALL success rate: {stats.sr:.3f} over {stats.n} episodes "
          f"({stats.num_success}/{stats.n})")
    return
  print("LEGACY DIAGNOSTIC: fixed-window evaluation may count reset retries")
