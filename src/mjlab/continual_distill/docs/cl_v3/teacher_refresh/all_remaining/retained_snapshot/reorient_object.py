    ...
    if ph == P_CLOSE:
      ...
      if self._closing[i]:
        ...
        if self._close_k[i] >= CLOSE_SQUEEZE:
          ...
          self._goto(i, P_LIFT)
          # _goto clears the phase-local closing flag. Keep the acquired grasp
          # closed on this transition instead of issuing a one-step open pulse.
          return self._guard(err, q_abs), rot, GRIPPER_CLOSED
