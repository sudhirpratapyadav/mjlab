...
from mjlab.continual_distill.classical.base import HOME_QPOS, ClassicalPolicyBase, lowest_hand_z
...
class LiftCubeClassicalPolicy(LiftObjectClassicalPolicy):
  ...
  def _target_error(self, i: int, obs_i: np.ndarray):
    ...
    err, rot, grip = super()._target_error(i, obs_i)
    q_abs = self.default_qpos + obs_i[:9]
    ee_pos, ee_rot = self._fk(q_abs)
    if phase in (1, 2):
      # Use the arm's height, independent of whether a missed cube moved.
      # Brake before the floor so servo lag cannot spend the grasp clearance.
      descent_rate = 0.006 if ee_pos[2] < 0.12 else 0.02
      err[2] = max(self.GRASP_SITE_Z - ee_pos[2], -descent_rate)
    low = lowest_hand_z(ee_pos, ee_rot, float(max(q_abs[7], q_abs[8])))
    err[2] = max(err[2], 0.012 - low)
    return err, rot, grip
