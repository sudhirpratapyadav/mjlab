  ...
  def _target_error(self, i: int, obs_i: np.ndarray):
    ...
    if self._phase[i] == 0:
      ...
      if self._settle[i] >= SETTLE or self._phase_steps[i] > SEAT_TIMEOUT:
        ...
        self._anchor[i] = self._fk(self.default_qpos + obs_i[:9])[0] + face + push_dir * PUNCH_DEPTH
        self._phase[i] = 2
        self._phase_steps[i] = 0
    elif self._phase[i] == 2:
      # PUNCH: drive at a fixed point well inside the block's geometry.
      pos_err = self._anchor[i] - self._fk(self.default_qpos + obs_i[:9])[0]
      self._punch_steps[i] += 1
      if self._punch_steps[i] >= PUNCH_STEPS or abs(_object_x_axis(obs_i)[2]) > 0.8:
        self._phase[i] = 3
        self._phase_steps[i] = 0
    else:
