...
# -- pinch --------------------------------------------------------------------
PINCH_U = -0.09  # pinch point along the shaft from the body origin (CoM is at ~0.026)
HOVER_Z = 0.16
PINCH_SITE_Z = 0.026  # the empirically safe site floor (terminations seen at 0.020-0.022); pad bottom 0.018
FLOOR_MIN_Z = 0.026  # legacy site-height floor (kept for reference)
PAD_CLEARANCE = 0.006  # required clearance of the LOWEST hand point above the ground
...
DESCENT_RATE = 0.015
DESCENT_RATE_LOW = 0.006  # below DESCENT_SLOW_Z: keeps the servo lag (and so the command lead) near zero
...
class ToolBase(ClassicalPolicyBase):
  ...
  def _target_error(self, i: int, obs_i: np.ndarray):
    ...
    self.cmd_lead_max = 0.0 if self._phase[i] == P_DESCEND else 0.12
...
from gravity import GravitySupport
class ToolPullClassicalPolicy(GravitySupport,ToolBase):
  def _target_error(self,i,obs):
    if 9 <= self._phase[i] <= 11:
      self._rewind(i)
    self.posture_weight = 0.0003 if 4 <= self._phase[i] <= 7 else 0.005
    return super()._target_error(i,obs)
