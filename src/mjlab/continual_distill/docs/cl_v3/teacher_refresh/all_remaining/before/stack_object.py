...
class StackObjectClassicalPolicy(GraspTransportPolicy):
  ...
  #
  # CL-V3 (W1-G, 2026-09-09): the diagnose trace showed the loss is in DESCEND (the
  # hand lands on the cube and shoves it 30-55 mm at 0.6-0.85 m/s before the close),
  # not in CLOSE. Lift-Cube's grasp phases are reused verbatim (grasp_style="lift"),
  # with a jump-based reset detector (the "home" test is dead under +-10 deg joint
  # noise), an unconditional CARRY escape and retries from DONE.
  grasp_style = "lift"
  reset_detect = "jump"
  carry_timeout = 90
