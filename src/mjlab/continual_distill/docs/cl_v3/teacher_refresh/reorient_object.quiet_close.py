...
class ReorientObjectClassicalPolicy(ClassicalPolicyBase):
  ...
  def reset(self, env_ids=None) -> None:
    ...
    if env_ids is None:
      ...
      self._close_q = np.full((n, 7), np.nan)
  ...
  def _act_single(self, i: int, obs_i: np.ndarray) -> np.ndarray:
    ...
    if ph <= P_LIFT:
      ...
      self._q_cmd[i][6] = q7
    if ph == P_CLOSE and self._closing[i]:
      # The object is stationary during the squeeze. Hold the arm command rather
      # than moving the pads in response to fresh joint/position noise each step.
      if np.isnan(self._close_q[i]).any():
        self._close_q[i] = self._q_cmd[i]
      self._q_cmd[i] = self._close_q[i]
      a[:7] = (self._close_q[i] - self.default_qpos[:7]) / FRANKA_ACTION_SCALE
    return a
