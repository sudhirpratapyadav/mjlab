...
class PegInsertionClassicalPolicy(GraspTransportPolicy):
  ...
  def _act_single(self, i, obs_i):
    ...
    # A home-posture bias that is harmless in free space leaves several degrees
    # of wrist error, exceeding the insertion clearance. Prioritize the actual
    # peg pose during transport/insertion while preserving the acquisition servo.
    self.posture_weight = 0.0003 if self._phase[i] in (P_CARRY, P_PLACE, P_RELEASE) else 0.005
    if self.carry_cmd_ref and self._phase[i] in (P_LIFT, P_CARRY):
  ...
  def _approach_rot(self, i: int, obs_i: np.ndarray):
    ...
    if self.grasp_pitch <= 0.0:
      nominal = super()._approach_rot(i, obs_i)
      if self._phase[i] == P_RELEASE and np.isfinite(self._insert_rot[i]).all():
        return self._insert_rot[i]
      if self._phase[i] not in (P_CARRY, P_PLACE):
        return nominal
      q = obs_i[self.quat_slice].copy()
      ...
      if peg[2, 2] < 0.8:
        return nominal
      _, hand = self._fk(self.default_qpos + obs_i[:9])
      yaw = np.arctan2(peg[1, 0], peg[0, 0])
      yaw = self.hole_yaw + np.round((yaw - self.hole_yaw) / self.yaw_symmetry) * self.yaw_symmetry
      c, s = np.cos(yaw), np.sin(yaw)
      desired_peg = np.array([[c, -s, 0], [s, c, 0], [0, 0, 1]])
      target = desired_peg @ peg.T @ hand
      if not np.isfinite(self._insert_rot[i]).all():
        self._insert_rot[i] = nominal
      u, _, vt = np.linalg.svd(0.8 * self._insert_rot[i] + 0.2 * target)
      self._insert_rot[i] = u @ vt
      return self._insert_rot[i]
  ...
  def reset(self, env_ids=None) -> None:
    super().reset(env_ids)
    if env_ids is None:
      self._insert_rot = np.full((self.num_envs, 3, 3), np.nan)
      ...
      self._insert_rot[env_ids] = np.nan
  ...
  def _place(self, i, obs_i, rot):
    ...
    q = obs_i[self.quat_slice]
    q = q / max(np.linalg.norm(q), 1e-8)
    upright = 1.0 - 2.0 * (q[1] ** 2 + q[2] ** 2) > np.cos(np.deg2rad(4.0))
    ...
    if abs(z_err) < 0.008 and lat < self.place_xy_tol and upright:
      self._settle[i] += 1
    else:
      self._settle[i] = 0
