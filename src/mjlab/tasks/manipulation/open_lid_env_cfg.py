...
from mjlab.tasks.manipulation.mdp import OpenLidCommandCfg
...
def make_open_lid_env_cfg() -> ManagerBasedRlEnvCfg:
  """Open a hinged lid against gravity (vertical-arc-under-gravity motion profile).
  ...
  events = {
    ...
    "reset_lid_position": EventTermCfg(
      func=mdp.reset_root_state_uniform,
      mode="reset",
      params={
        "pose_range": {
  ...
  # Collision sensor for end-effector to lid static-body contact.
  ee_lid_collision_cfg = ContactSensorCfg(
    name="ee_lid_collision",
    primary=ContactMatch(
      mode="subtree",
      pattern="",  # Set per-robot (e.g., "link7" for Franka).
      entity="robot",
    ...
    secondary=ContactMatch(
      mode="geom",
      pattern="lid_body",  # Static body geom, not the moving part
      entity="lid",
    ),
    fields=("found",),
    reduce="none",
  ...
  return ManagerBasedRlEnvCfg(
    scene=SceneCfg(
      ...
      num_envs=1,
      env_spacing=1.0,
      sensors=(ee_ground_collision_cfg, ee_lid_collision_cfg),
    ),
    ...
    sim=SimulationCfg(
      ...
      mujoco=MujocoCfg(
        ...
        impratio=10,
        cone="elliptic",
        gravity=(0.0, 0.0, -9.81),  # ENABLED: this task's premise is gravity
      ),
    ),
    decimation=4,
