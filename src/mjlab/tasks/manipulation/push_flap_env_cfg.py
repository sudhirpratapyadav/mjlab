...
from mjlab.tasks.manipulation.mdp import PushFlapCommandCfg
...
def make_push_flap_env_cfg() -> ManagerBasedRlEnvCfg:
  """Swing a handle-less hinged flap by face-pushing along its arc.

  Motion profile: NON-PREHENSILE HINGE-ARC PUSH. Same vertical hinge axis as the
  door, but no handle: the fingertips push the flat panel and must re-orient along
  the arc as the contact normal rotates with it. The hinge sign convention (the
  push drives the joint NEGATIVE) is documented on PushFlapCommandCfg.

  Part of the Class A Wave-1 expansion; see
  ``continual_distill/docs/benchmark/CATALOG_100_TASKS.md`` (T21).
  """
  ...
  events = {
    ...
    "reset_flap_position": EventTermCfg(
      func=mdp.reset_root_state_uniform,
      mode="reset",
      params={
        "pose_range": {
  ...
  # Collision sensor for end-effector to flap static-body contact.
  ee_flap_collision_cfg = ContactSensorCfg(
    name="ee_flap_collision",
    primary=ContactMatch(
      mode="subtree",
      pattern="",  # Set per-robot (e.g., "link7" for Franka).
      entity="robot",
    ...
    secondary=ContactMatch(
      mode="geom",
      pattern="flap_body",  # Static body geom, not the moving part
      entity="flap",
    ),
    fields=("found",),
    reduce="none",
  ...
  rewards = {
    ...
    "no_object_collision": RewardTermCfg(
      ...
      weight=0.25,
      params={"sensor_name": "ee_flap_collision"},
    ),
    # Regularization
  ...
  return ManagerBasedRlEnvCfg(
    scene=SceneCfg(
      ...
      num_envs=1,
      env_spacing=1.0,
      sensors=(ee_ground_collision_cfg, ee_flap_collision_cfg),
    ),
