...
from mjlab.tasks.manipulation.mdp import SlideWindowCommandCfg
...
def make_slide_window_env_cfg() -> ManagerBasedRlEnvCfg:
  """Slide a pane laterally (lateral face-push motion profile).
  ...
  policy_terms = {
    ...
    "object_pos": ObservationTermCfg(
      ...
      params={"object_asset_name": "window"},
      noise=Unoise(n_min=-0.01, n_max=0.01),
    ),
  ...
  commands: dict[str, CommandTermCfg] = {
    "slide_window": SlideWindowCommandCfg(
      asset_name="window",
  ...
  events = {
    ...
    "reset_window_position": EventTermCfg(
      func=mdp.reset_root_state_uniform,
      mode="reset",
      params={
        "pose_range": {
        ...
        "velocity_range": {},
        "asset_cfg": SceneEntityCfg("window"),
      },
    ),
  ...
  # Collision sensor for end-effector to window static-body contact.
  ee_window_collision_cfg = ContactSensorCfg(
    name="ee_window_collision",
    primary=ContactMatch(
      mode="subtree",
      pattern="",  # Set per-robot (e.g., "link7" for Franka).
      entity="robot",
    ...
    secondary=ContactMatch(
      mode="geom",
      pattern="window_body",  # Static body geom, not the moving part
      entity="window",
    ),
    fields=("found",),
    reduce="none",
  ...
  rewards = {
    ...
    "no_object_collision": RewardTermCfg(
      ...
      weight=0.25,
      params={"sensor_name": "ee_window_collision"},
    ),
    # Regularization
  ...
  return ManagerBasedRlEnvCfg(
    scene=SceneCfg(
      ...
      num_envs=1,
      env_spacing=1.0,
      sensors=(ee_ground_collision_cfg, ee_window_collision_cfg),
    ),
