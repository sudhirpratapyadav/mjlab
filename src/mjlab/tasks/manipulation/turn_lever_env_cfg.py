...
from mjlab.tasks.manipulation.mdp import TurnLeverCommandCfg
...
def make_turn_lever_env_cfg() -> ManagerBasedRlEnvCfg:
  """Turn a lever about the approach axis (wrist-rotation motion profile).
  ...
  policy_terms = {
    ...
    "object_pos": ObservationTermCfg(
      ...
      params={"object_asset_name": "lever"},
      noise=Unoise(n_min=-0.01, n_max=0.01),
    ),
  ...
  commands: dict[str, CommandTermCfg] = {
    "turn_lever": TurnLeverCommandCfg(
      asset_name="lever",
  ...
  events = {
    ...
    # Randomize lever position 
    "reset_lever_position": EventTermCfg(
      func=mdp.reset_root_state_uniform,
      mode="reset",
      params={
        "pose_range": {
        ...
        "velocity_range": {},
        "asset_cfg": SceneEntityCfg("lever"),
      },
    ),
  ...
  # Collision sensor for end-effector to lever static-body contact.
  ee_lever_collision_cfg = ContactSensorCfg(
    name="ee_lever_collision",
    primary=ContactMatch(
      mode="subtree",
      pattern="",  # Set per-robot (e.g., "link7" for Franka).
      entity="robot",
    ...
    secondary=ContactMatch(
      mode="geom",
      pattern="lever_body",  # Static body geom, not the moving part
      entity="lever",
    ),
    fields=("found",),
    reduce="none",
  ...
  return ManagerBasedRlEnvCfg(
    scene=SceneCfg(
      ...
      num_envs=1,
      env_spacing=1.0,
      sensors=(ee_ground_collision_cfg, ee_lever_collision_cfg),
    ),
