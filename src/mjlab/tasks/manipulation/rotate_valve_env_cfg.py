...
from mjlab.tasks.manipulation.mdp import RotateValveCommandCfg
...
def make_rotate_valve_env_cfg() -> ManagerBasedRlEnvCfg:
  """Rotate a valve wheel 270 deg (multi-cycle regrasp motion profile).
  ...
  policy_terms = {
    ...
    "object_pos": ObservationTermCfg(
      ...
      params={"object_asset_name": "valve"},
      noise=Unoise(n_min=-0.01, n_max=0.01),
    ),
  ...
  commands: dict[str, CommandTermCfg] = {
    "rotate_valve": RotateValveCommandCfg(
      asset_name="valve",
  ...
  events = {
    ...
    # Randomize valve position 
    "reset_valve_position": EventTermCfg(
      func=mdp.reset_root_state_uniform,
      mode="reset",
      params={
        "pose_range": {
  ...
  # Collision sensor for end-effector to valve static-body contact.
  ee_valve_collision_cfg = ContactSensorCfg(
    name="ee_valve_collision",
    primary=ContactMatch(
      mode="subtree",
      pattern="",  # Set per-robot (e.g., "link7" for Franka).
      entity="robot",
    ...
    secondary=ContactMatch(
      mode="geom",
      pattern="valve_body",  # Static body geom, not the moving part
      entity="valve",
    ),
    fields=("found",),
    reduce="none",
  ...
  return ManagerBasedRlEnvCfg(
    scene=SceneCfg(
      ...
      num_envs=1,
      env_spacing=1.0,
      sensors=(ee_ground_collision_cfg, ee_valve_collision_cfg),
    ),
