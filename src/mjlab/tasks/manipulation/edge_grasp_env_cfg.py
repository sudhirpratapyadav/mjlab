...
from mjlab.tasks.manipulation.mdp import EdgeGraspCommandCfg
...
def make_edge_grasp_env_cfg() -> ManagerBasedRlEnvCfg:
  """Slide a thin plate to a ledge edge, pinch it at the overhang, lift it clear.

  Motion profile: EXTRINSIC-DEXTERITY EDGE GRASP. The plate is unspannable (both
  widths exceed the aperture) and unpinchable while flat (16 mm thick, no finger
  clearance underneath); the ledge edge is the fixture that exposes a graspable
  face. EdgeGraspCommand owns the ledge (static mocap, written per-env) and the
  plate spawn; success is the plate held ABOVE the ledge top, which sliding alone
  cannot produce — pushed off the edge it falls BELOW the top.

  Part of the Class A Wave-1 expansion; see
  ``continual_distill/docs/benchmark/CATALOG_100_TASKS.md`` (T23).
  """
  ...
  policy_terms = {
    ...
    "object_pos": ObservationTermCfg(
      ...
      params={"object_asset_name": "plate"},
      noise=Unoise(n_min=-0.01, n_max=0.01),
    ),
  ...
  commands: dict[str, CommandTermCfg] = {
    "edge_grasp": EdgeGraspCommandCfg(
      asset_name="plate",
      robot_asset_cfg=SceneEntityCfg("robot", site_names=()),  # Set per-robot
  ...
  terminations = {
    ...
    "object_out_of_bounds": TerminationTermCfg(
      ...
      params={
        "object_name": "plate",
        "x_bounds": (0.0, 1.0),
        "y_bounds": (-0.5, 0.5),  # A dropped plate lands near the ledge; flung = fail
      },
    ),
  }
