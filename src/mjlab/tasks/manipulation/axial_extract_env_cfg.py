...
from mjlab.tasks.manipulation.mdp import AxialExtractCommandCfg
...
def make_axial_extract_env_cfg() -> ManagerBasedRlEnvCfg:
  """Pull a friction-fit plug axially out of its socket (breakaway extraction).

  Motion profile: FRICTION-BREAKAWAY EXTRACTION. The plug's slide joint carries a
  ~4 N frictionloss: nothing moves until the pinch-and-pull exceeds breakaway,
  then the plug must be guided straight up out of the bore. Contrast the drawer's
  low-friction hooked glide. Gravity stays disabled like the other mocap-mounted
  mechanisms — the resistance is the joint friction, a single tunable number.

  Part of the Class A Wave-1 expansion; see
  ``continual_distill/docs/benchmark/CATALOG_100_TASKS.md`` (T22).
  """
  ...
  commands: dict[str, CommandTermCfg] = {
    "axial_extract": AxialExtractCommandCfg(
      asset_name="plug",
  ...
  events = {
    ...
    "reset_plug_position": EventTermCfg(
      func=mdp.reset_root_state_uniform,
      mode="reset",
      params={
        "pose_range": {
  ...
  # Collision sensor for end-effector to plug socket contact.
  ee_plug_collision_cfg = ContactSensorCfg(
    name="ee_plug_collision",
    primary=ContactMatch(
      mode="subtree",
      pattern="",  # Set per-robot (e.g., "link7" for Franka).
      entity="robot",
    ...
    secondary=ContactMatch(
      mode="geom",
      pattern="plug_body|plug_wall_.*",  # Static body geom, not the moving part
      entity="plug",
    ),
    fields=("found",),
    reduce="none",
  ...
  return ManagerBasedRlEnvCfg(
    scene=SceneCfg(
      ...
      num_envs=1,
      env_spacing=1.0,
      sensors=(ee_ground_collision_cfg, ee_plug_collision_cfg),
    ),
    ...
    sim=SimulationCfg(
      ...
      mujoco=MujocoCfg(
        ...
        impratio=10,
        cone="elliptic",
        gravity=(0.0, 0.0, 0.0),  # Matches the door/drawer/button articulation
        # tasks, which all disable gravity. Deliberate inheritance, not a debug
        # leftover: the mechanism is anchored to a mocap base, so gravity only
        # loads the arm. Open-Lid overrides this — see CLASS_A_EXPANSION.md.
      ),
    ),
    decimation=4,
    episode_length_s=4.0,  # Breakaway + guided withdrawal needs longer than a stroke
  )
