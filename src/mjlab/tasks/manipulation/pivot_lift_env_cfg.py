...
from mjlab.tasks.manipulation.mdp import PivotLiftCommandCfg
...
def make_pivot_lift_env_cfg() -> ManagerBasedRlEnvCfg:
  """Pivot an ungraspable flat board against a wall, then grasp and lift it.

  Motion profile: PIVOT-AGAINST-WALL GRASP (extrinsic dexterity). Flat on the
  ground the board is unspannable and offers no finger clearance; pushed into the
  wall it pivots up onto an edge, exposing its 20 mm thickness for a pinch. The
  goal is an airborne point (LiftingCommand semantics), so sliding can never
  satisfy it — the wall is the only route to a grasp. PivotLiftCommand places the
  wall (static mocap) per-env.

  Part of the Class A Wave-1 expansion; see
  ``continual_distill/docs/benchmark/CATALOG_100_TASKS.md`` (T24).
  """
  ...
  policy_terms = {
    ...
    "object_pos": ObservationTermCfg(
      ...
      params={"object_asset_name": "board"},
      noise=Unoise(n_min=-0.01, n_max=0.01),
    ),
  ...
  commands: dict[str, CommandTermCfg] = {
    "pivot_lift": PivotLiftCommandCfg(
      asset_name="board",
      robot_asset_cfg=SceneEntityCfg("robot", site_names=()),  # Set per-robot
      ...
      object_pose_range=PivotLiftCommandCfg.ObjectPoseRangeCfg(
        # Paired with PivotLiftCommandCfg.wall_spawn_range (inner face 0.508): the
        # board's yaw-swept +x half-extent is 0.0756, so this band never spawns into
        # the wall and leaves 22-90 mm of push travel. The band was moved OUT from
        # 0.32-0.38 in CL-V2 (W1-c) to lift the pusher's approach point
        # (board_x - 0.076) out of the GRASP_RADIAL_MIN dead zone — the cl25 M4
        # blocker; see the wall_spawn_range docstring, constraint 3.
        x=(0.38, 0.41),
        y=(-0.12, 0.12),
        z=(0.0101, 0.0101),  # Board half-thickness (scanned board): resting flat
        # Narrowed from +-0.3: the board is now 219 mm long across the push direction,
        # and +-0.3 rad of yaw alone projects 32 mm onto +x — more than the push travel.
        yaw=(-0.15, 0.15),
      ),
      target_position_range=PivotLiftCommandCfg.TargetPositionRangeCfg(
        x=(0.32, 0.48),
        y=(-0.15, 0.15),
        z=(0.15, 0.30),  # Airborne: only a grasped board can get here
      ),
    )
  }
