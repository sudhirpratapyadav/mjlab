...
from mjlab.tasks.manipulation.mdp import FlipSwitchCommandCfg
...
def make_flip_switch_env_cfg() -> ManagerBasedRlEnvCfg:
  """Flip a detented toggle past centre (ballistic-commit motion profile).

  Part of the Class A motion-profile expansion; see
  ``continual_distill/docs/benchmark/CLASS_A_EXPANSION.md``.
  """
  ...
  policy_terms = {
    ...
    "object_pos": ObservationTermCfg(
      ...
      params={"object_asset_name": "switch"},
      noise=Unoise(n_min=-0.01, n_max=0.01),
    ),
  ...
  commands: dict[str, CommandTermCfg] = {
    "flip_switch": FlipSwitchCommandCfg(
      asset_name="switch",
  ...
  events = {
    ...
    # Randomize switch position 
    "reset_switch_position": EventTermCfg(
      func=mdp.reset_root_state_uniform,
      mode="reset",
      params={
        "pose_range": {
        ...
        "velocity_range": {},
        "asset_cfg": SceneEntityCfg("switch"),
      },
    ),
  ...
  # Collision sensor for end-effector to switch static-body contact.
  ee_switch_collision_cfg = ContactSensorCfg(
    name="ee_switch_collision",
    primary=ContactMatch(
      mode="subtree",
      pattern="",  # Set per-robot (e.g., "link7" for Franka).
      entity="robot",
    ...
    secondary=ContactMatch(
      mode="geom",
      pattern="switch_body",  # Static body geom, not the moving part
      entity="switch",
    ),
    fields=("found",),
    reduce="none",
  ...
  return ManagerBasedRlEnvCfg(
    scene=SceneCfg(
      ...
      num_envs=1,
      env_spacing=1.0,
      sensors=(ee_ground_collision_cfg, ee_switch_collision_cfg),
    ),
    ...
    sim=SimulationCfg(
      ...
      mujoco=MujocoCfg(
        ...
        impratio=10,
        cone="elliptic",
        gravity=(0.0, 0.0, -9.81),  # ENABLED: the toggle's bistable
        # over-centre detent is gravity-driven (a weighted inverted-pendulum
        # lever). With gravity off the switch has no preferred state and the
        # "commit past centre" motion profile disappears entirely.
      ),
    ),
    decimation=4,
