...
from mjlab.tasks.manipulation.mdp import ReorientObjectCommandCfg
...
def make_topple_block_env_cfg() -> ManagerBasedRlEnvCfg:
  """Topple an ungraspable standing block so a designated face lands down.

  Motion profile: NON-PREHENSILE TOPPLE. Every face of the block is wider than the
  gripper aperture, so force closure is impossible by construction; the only route
  to the goal orientation is to poke above the centre of mass and tip the block
  over an edge. Reuses ReorientObjectCommand with body_axis=(1,0,0) and
  symmetric_axis=True: the body x-axis is HORIZONTAL at spawn and must end
  vertical (either sign — the two opposite landing faces are the same outcome).

  Part of the Class A Wave-1 expansion; see
  ``continual_distill/docs/benchmark/CATALOG_100_TASKS.md`` (T20).
  """
  ...
  policy_terms = {
    ...
    "object_pos": ObservationTermCfg(
      ...
      params={"object_asset_name": "block"},
      noise=Unoise(n_min=-0.01, n_max=0.01),
    ),
  ...
  commands: dict[str, CommandTermCfg] = {
    "topple_block": ReorientObjectCommandCfg(
      asset_name="block",
      robot_asset_cfg=SceneEntityCfg("robot", site_names=()),  # Set per-robot
      ...
      max_drift=0.30,  # A topple travels half_x + half_z = 0.155 m; flinging further fails
      marker_z_offset=0.22,
    )
  }
