...
def make_drag_pull_env_cfg() -> ManagerBasedRlEnvCfg:
  """Drag a far object back toward the base (engagement-inverted planar transport).

  Motion profile: PLANAR DRAG-PULL. The push task drives the object AWAY — the
  fingertips engage the near face and the arm extends. Here the object spawns at
  the far edge of the envelope and the goal sits in the near zone, so the
  fingertips must reach OVER and BEHIND the object and the arm retracts while
  keeping engagement on the far face. Same command machinery as push
  (PushingCommand), inverted spawn/goal geometry; the concrete cfg enforces
  goal-nearer-than-object.

  Part of the Class A Wave-1 expansion; see
  ``continual_distill/docs/benchmark/CATALOG_100_TASKS.md`` (T17).
  """
  ...
  commands: dict[str, CommandTermCfg] = {
    "drag_pull": PushingCommandCfg(
      ...
      # Inverted geometry vs push: object FAR, goal NEAR (see concrete cfg for the
      # audited ranges; these defaults keep the base self-consistent).
