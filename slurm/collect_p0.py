...
LOGS = REPO / "logs"
DOC = REPO / "src/mjlab/continual_distill/docs/P0_EXPERIMENTS.md"

SUCC_RE = re.compile(r"^\s+- (\w+): KL=[\d.eE+-]+ \| .*StudentSucc=([\d.]+)", re.M)

RUNS = (
    [(f"nosi_{o}_s{s}", "P0-1") for o in ("best", "worst") for s in (0, 1, 2)]
    + [(f"joint_s{s}", "P0-2") for s in (0, 1, 2)]
    + [(f"w8192_lr{lr}_s{s}", "P0-3") for lr in ("1e5", "5e6", "1e6") for s in (0, 1, 2)]
)
