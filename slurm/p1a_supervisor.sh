...
#
# Every 10 min it:
#   1. restarts run_p1a.sh if the scheduler died (it is idempotent — finished runs
...
#
# is set so the 3 joint runs are dispatched alongside the other 15; they
...
while true; do
  ...
  if ! pgrep -f "run_p1a.sh $HOLDER" > /dev/null; then
    echo "[$(date +%F' '%T)] scheduler not running -> (re)starting"
    nohup bash "$REPO/slurm/run_p1a.sh" "$HOLDER" "$PROJ" \
      >> "$REPO/logs/p0_scheduler.log" 2>&1 &
  fi

  "$REPO/.venv/bin/python" "$REPO/slurm/collect_p1a.py" --write \
    > "$REPO/logs/p1a_status.txt" 2>&1
  ndone=$(grep -oP 'complete: \K\d+' "$REPO/logs/p1a_status.txt" | head -1)
  nbusy=$(srun --jobid="$HOLDER" --overlap bash -c \
    "nvidia-smi --query-compute-apps=gpu_uuid --format=csv,noheader" 2>/dev/null \
    | sort -u | wc -l)
  echo "[$(date +%F' '%T)] complete=${ndone:-?}/21  gpus_busy=${nbusy:-?}"

  # Surface runs that died without finishing — otherwise a crashloop looks like
  # "still training" forever.
  for f in "$REPO"/logs/cl_ewc_*.log "$REPO"/logs/cl_l2_*.log "$REPO"/logs/cl_lrw*.log; do
    [ -f "$f" ] || continue
  ...
  if [ "${ndone:-0}" -ge 18 ]; then
    echo "[$(date +%F' '%T)] ALL 21 P1-A RUNS COMPLETE."
    "$REPO/.venv/bin/python" "$REPO/slurm/collect_p1a.py" --write >> "$REPO/logs/p1a_status.txt" 2>&1
    break
  fi
